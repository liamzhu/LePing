...
public delegate void ConnectStatusCallBack(Net.NetworkState connectStstus);

public class Net : MonoSingleton<Net>, IHttpCallback
{
    ...
    private List<NetworkState> mStatusList = new List<NetworkState>();
    ...
    protected override void OnInit()
    {
        ...
        NetErrorCallback = (type, id, msg) => UIDialogMgr.Instance.ShowDialog(string.Format("Net error:{0}-{1}", id, msg));
        base.OnInit();
    }
    ...
    private void Dispatch(Net.NetworkState nType)
    {
        //Debug.Log("NetworkStateCallback");
    ...
    private void SocketRequest(GameAction gameAction, ActionParam actionParam, IHeadFormater formater, bool bShowLoading)
    {
        if (mSocket == null)
        {
            ...
            mSocket.mConnectStatusCallback = ConnectStatusChange;
        }
